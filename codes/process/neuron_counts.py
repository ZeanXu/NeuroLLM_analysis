...
def process_layer(task_name, word_id, layer_name):
    ...
    return [task_name, word_id, layer_name, data.shape[2]]
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame(all_results, columns=['task_name', 'word_id', 'layer_name', 'neuron_count'])
    df.to_csv('/nfs/xuhan/xyh/analysis/sparse/neuron_counts.csv', index=False)
