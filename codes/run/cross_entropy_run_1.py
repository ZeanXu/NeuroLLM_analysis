...
def calculate_ks_crossentropy(x, y):
    # 计算x和y的概率分布
    px_prob = calculate_frequency(x)
    py_prob = calculate_frequency(y)
    
    # 获取所有唯一项的集合
    all_items = set(px_prob.keys()).union(set(py_prob.keys()))
    
    # 计算交叉熵 H(P, Q)
    cross_entropy = 0
    for item in all_items:
        px = px_prob.get(item, 0)
        py = py_prob.get(item, 1e-10)  # 避免对数中的零概率
        if px > 0:  # 只有在 px > 0 时才考虑
            cross_entropy -= px * np.log(py)
...
def process_layer(task_name, word_id, layer_name_1, layer_name_2):
    ...
    for pair in range(ce_shape):
        ...
        ce = calculate_ks_crossentropy(x, y)
        ce_matrix[pair] = ce

    print(f"task '{task_name}', word '{word_id}', layer '{layer_name_2}': Cross-Entropy average = {np.mean(ce_matrix)}")
    save_path = f"/nfs/xuhan/xyh/results/cross_entropy_new/{task_name}/{word_id}/{layer_name_2}.npy"
