...
def process_layer(args):
    ...
    for neuron in range(entropy_shape):
        shannon_entropy = knn_entropy(t[:, neuron].reshape(-1, 1)) # 可以手动调整knn里k的数值
    ...
    save_path = f"/nfs/xuhan/xyh/results/entropy/{task_name}/{word_id}/{layer_name}.npy"
